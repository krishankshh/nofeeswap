import MockERC20ABI from '../abis/MockERC20.json';
import NofeeOperatorLikeABI from '../abis/NofeeOperatorLike.json';

// These addresses are populated after deployment.
// Update with actual deployed addresses from chain/deployments/localhost.json
export const CONTRACT_ADDRESSES = {
  tokenA: '0x5FbDB2315678afecb367f032d93F642f64180aa3',
  tokenB: '0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512',
  operator: '0x9fE46736679d2D9a65F0992F2272dE9f3c7fa6e0',
};

export const ABIS = {
  MockERC20: MockERC20ABI.abi,
  NofeeOperatorLike: NofeeOperatorLikeABI.abi,
};

export const DEFAULT_POOL = {
  feeBps: 30,
};

export { MockERC20ABI, NofeeOperatorLikeABI };
